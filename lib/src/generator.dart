...
class CodeGenerator {
  static String _toCamelCase(String input) {
    ...
    if (RegExp(r'^\d').hasMatch(snakeCase)) {
  ...
  static String _generateEnumCase(SvgSymbol symbol, IconFontConfig config) {
    ...
    if (config.trimIconPrefix.isNotEmpty) {
      final prefixRegex = RegExp('^${config.trimIconPrefix}(.+?)\$');
      iconId = iconId.replaceFirst(prefixRegex, '\$1');
    }
  ...
  static String _generateSvgCase(SvgSymbol symbol, IconFontConfig config) {
    ...
    buffer.writeln('          <svg viewBox="${symbol.viewBox}" xmlns="http://www.w3.org/2000/svg">');
    ...
    for (int i = 0; i < symbol.paths.length; i++) {
      ...
      if (path.fill != null) {
        ...
        buffer.write(' fill="''' + getColor($i, color, colors, '${path.fill}') + '''"');
      } else {
  ...
  static Future<void> generateIconFont(List<SvgSymbol> symbols, IconFontConfig config) async {
    final names = <String>[];
